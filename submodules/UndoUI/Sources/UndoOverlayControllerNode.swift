...
final class UndoOverlayControllerNode: ViewControllerTracingNode {
    ...
    init(presentationData: PresentationData, content: UndoOverlayContent, elevatedLayout: Bool, placementPosition: UndoOverlayController.Position, blurred: Bool, action: @escaping (UndoOverlayAction) -> Bool, dismiss: @escaping () -> Void) {
        ...
        switch content {
            ...
            case let .audioRate(rate, text):
                ...
                displayUndo = false
                self.originalRemainingSeconds = 3
            case let .forward(savedMessages, text, jumpAvatar, actionText):
                self.avatarNode = nil
                self.iconNode = nil
                self.iconCheckNode = nil
    ...
    func animateIn(asReplacement: Bool) {
        if let iconNode = animationNode, let avatarView = jumpAvatarView {
            ...
            UIView.animate(withDuration: 0.1, delay: 0.25, animations: {
                iconNode.view.transform = .identity
            })

            UIView.animate(withDuration: 0.05, delay: 0.30, animations: {
                avatarCopy.alpha = 0
            })
    ...
    func animateOutWithReplacement(completion: @escaping () -> Void) {
        ...
        self.animationNode?.layer.animateAlpha(from: 1.0, to: 0.0, duration: 0.3, delay: 0.0, removeOnCompletion: false)
        self.animationNode?.layer.animateScale(from: 1.0, to: 0.1, duration: 0.3, removeOnCompletion: false)
    }
