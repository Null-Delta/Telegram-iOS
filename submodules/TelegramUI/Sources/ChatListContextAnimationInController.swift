...
public class ChatListContextAnimationInController: ContextAnimationInControllerProtocol {
    ...
    private func animateChatListItem(
        ...
        //contentNodeController.view.subviews[0].subviews[0].subviews[3]
        ...
        if let originalProjectedContentViewFrame = controller.originalProjectedContentViewFrame {
            ...
            if contentNodeController.controller is ChatControllerImpl,
               let navigationBar = navigationBar(from: contentNodeController.controller) {
                ...
                NavigationBarTitleCenter = contentNodeController.view.subviews[0].subviews[3].subviews[3].subviews[0].subviews[0].subviews[0].subviews[1].subviews[0].subviews[0].subviews[0].globalFrame!.center
            } else {
    ...
    private func animateForum(
        ...
        guard
            case let .controller(contentNodeController) = contentNode//,
            //let chatController = contentNodeController.controller as? ChatListControllerImpl
            //let navigationBar = chatController.navigationBar
        else {
            return
        }
        ...
        chatTitleView?.titleFont = Font.medium(titleFontSize)
        chatTitleView?.titleContent = chatTitleView?.titleContent
                                    
        controller.actionsContainerNode.layer.animateSpring(
            from: NSValue(cgPoint: CGPoint(x: startFrame.center.x - controller.actionsContainerNode.position.x, y: startFrame.center.y - controller.actionsContainerNode.position.y)),
